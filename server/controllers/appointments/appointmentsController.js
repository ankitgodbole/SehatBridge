...
const bookAppointmentByHospitalID = async (req, res) => {
  try {
    ...
    await Promise.all([hospital.save(), user.save()]);
...
const addEmergencyAppointment = asyncHandler(async (req, res) => {
  ...
  const hospitals = await Hospital.find({}, { lat: 1, long: 1, name: 1, _id: 1, address: 1, phone: 1 });
  let minTime = Infinity;
  let nearestHospital = null;

  for (let hospital of hospitals) {
    const route = await axios.get(
      `https://router.project-osrm.org/route/v1/driving/${userLong},${userLat};${hospital.long},${hospital.lat}?overview=false`
    );
  ...
  try {
    ...
    await Promise.all([hospital.save(), profile.save()]);
